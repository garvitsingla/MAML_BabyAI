    ...
    def gen_mission(self):
        ...
        if self._forced_mission is not None:
            ...
            if m:
                ...
                # Place at grid cell (5, 5)
                # self.grid.set(5, 5, target_obj)
                ...
                target_obj, _ = self.add_object(                    # adding the mission object in grid
                    i, j, obj_type, color
                )

                # Add distractors from all object/color combinations except exact target
                distractor_count = 0
...
class GoToObjDoorMissionEnv(GoToObjDoor):
    ...
    def gen_mission(self):
        ...
        if self._forced_mission is not None:
            # Example forced mission: "go to the red door"
            ...
            if m:
                ...
                if obj_type == "door":
                    ...
                    # Shuffle so target isn't always at same position
                    ...
                    objs.append(obj)
                    # Add distractors (other types/colors)
    ...
    def gen_mission(self):
        if self._forced_mission is not None:
            ...
            if m:
                ...
                if obj_type == "door":
                # ensure at least one door of the requested color exists
                    doors = []
                        ...
                        for j in range(self.num_rows):
                            ...
                            for door in room.doors:
                                if door and door.color == color:
                                    doors.append(door)
                    if not doors:
                        # put one in a known location; adjust (i,j) if you prefer
                    ...
                    # Add distractors
                    # Add distractors from ALL (type, color) pairs except the exact target
                    choices = [(t, c) for t in OBJECTS for c in COLORS
                            if not (t == obj_type and c == color)]
                    placed = 0
...
class OpenDoorMissionEnv(Open):
    ...
    def __init__(self, debug = False, **kwargs):
        ...
        super().__init__(**kwargs)
        self._forced_mission = None
        # self.distractors = kwargs.get("dists", 3)
        # self.render_mode = kwargs.get('render_mode', 'human')
    ...
    def gen_mission(self):
        # Use the center cell (1,1) like BabyAI examples; works for 3x3 (interior) and 2x2 (corner)
        i = j = 1
        ...
        # Choose target color (forced or random)
        target_color = color
...
class OpenDoorLocMissionEnv(Open):
    ...
    def gen_mission(self):
        # Place agent in (1,1); valid-sides guards placement for 2x2/3x3
        i = j = 1
        self.place_agent(i, j)
        ...
        sides = self._valid_sides(i, j)
        self.np_random.shuffle(sides)
        first_door = None  # for type reference in ObjDesc

        if select_by == "color":
            # target color: forced or random
